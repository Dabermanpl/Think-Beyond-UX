...
export function Navbar() {
  ...
  return (
    <nav className="fixed top-6 left-0 right-0 z-50 px-6 pointer-events-none">
          ...
          {navLinks.map((link) => (
            <Link 
              ...
              className={`text-xs font-bold uppercase tracking-wider transition-colors ${
                activeSection === link.id ? "text-black" : "text-text-muted hover:text-black"
              }`}
            >
              {link.label}
            </Link>
      ...
      <div 
        className={`md:hidden absolute top-24 left-6 right-6 bg-white/95 backdrop-blur-md border border-zinc-200 rounded-[32px] shadow-2xl overflow-hidden transition-all duration-300 pointer-events-auto ${
        ...
        <div className="p-4 flex flex-col gap-2">
          {navLinks.map((link) => (
            <Link 
              ...
              href={link.href}
              onClick={closeMenu}
              className={`px-6 py-4 rounded-2xl text-sm font-bold uppercase tracking-wider transition-all ${
                activeSection === link.id ? "bg-black text-white" : "text-text-muted hover:bg-zinc-50 hover:text-black"
              }`}
            >
              {link.label}
            </Link>
